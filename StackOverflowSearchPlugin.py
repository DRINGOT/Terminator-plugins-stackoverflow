...
# Modified by DRINGOT for Stack Overflow research 2020/05/14

# AVAILABLE must contain a list of all the classes that you want exposed
AVAILABLE = ['StackOverflowSearchPlugin']
...
class StackOverflowSearchPlugin(plugin.Plugin):
    capabilities = ['terminal_menu']

    def do_search(self, searchMenu):
        """Launch Stack Overflow search for string"""
        if not self.searchstring:
            return
        base_uri = "https://stackoverflow.com/search?q=%s"
    ...
    def callback(self, menuitems, menu, terminal):
        ...
        if self.searchstring:
            ...
            item.set_label("Search Stack Overflow for \"%s\"" % displaystring)
            item.set_sensitive(True)
        else:
            item.set_label("Stack Overflow")
